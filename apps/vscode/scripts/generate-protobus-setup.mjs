...
async function generateWebviewProtobusClients(protobusServices) {
	...
	for (const [serviceName, def] of Object.entries(protobusServices)) {
		...
		return this.makeUnaryRequest("${rpcName}", request, ${requestType}.toJSON, ${responseType}.fromJSON)
				...
				rpcs.push(`    static ${rpcName}(request: ${requestType}, callbacks: Callbacks<${responseType}>, options?: StreamingOptions): ()=>void {
		return this.makeStreamingRequest("${rpcName}", request, ${requestType}.toJSON, ${responseType}.fromJSON, callbacks, options)
	}`)
			}
		}
