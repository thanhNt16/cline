...
`
	...
	beforeEach(async () => {
		...
		// Create .cellockai/rules/hooks subdirectory structure
		tempDir = path.join(baseTempDir, ".cellockai", "rules", "hooks")
		await fs.mkdir(tempDir, { recursive: true })
		testHandler = createTestHandler()
		mockMessages = []

		// Mock StateManager to return baseTempDir as workspace root
		// This allows HookFactory to find hooks in baseTempDir/.cellockai/rules/hooks/
