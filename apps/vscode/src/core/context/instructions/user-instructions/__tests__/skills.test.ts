...
describe("Skills Utility Functions", () => {
	...
	const GLOBAL_SKILLS_DIR = path.join("/home", "user", ".cellockai", "skills")
...
Use analogies and ASCII diagrams when explaining code.`)
		...
		it("should discover skills from project .cellockai/skills directory", async () => {
			const clineSkillsDir = path.join(TEST_CWD, ".cellockai", "skills")
...
describe("setSkillDisabledInFrontmatter", () => {
	...
	it("writes disabled: true to the SKILL.md when disabling a disk skill", async () => {
		const skillPath = path.join("/home", "user", ".cellockai", "skills", "s", "SKILL.md")
