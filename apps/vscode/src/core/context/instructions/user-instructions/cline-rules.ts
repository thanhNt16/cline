...
/**
 * Sub-directories that hold non-rule config and must not be surfaced as
 * rules: upstream's `.clinerules` layout and CellockAI's `.cellockai/rules`
 * layout (workflows and hooks live inside the rules directory).
 */
...
export async function refreshClineRulesToggles(
	...
	// CellockAI also loads rules from <workspace>/.cellockai/rules, alongside
	// the upstream layouts the shared SDK resolver covers (.clinerules and
	// .cline/rules).
	const updatedLocalToggles = await synchronizeRuleTogglesAcrossDirectories(
		[...resolveWorkspaceRulesConfigPaths(workingDirectory), path.resolve(workingDirectory, GlobalFileNames.clineRules)],
