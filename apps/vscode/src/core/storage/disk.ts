 ...
 * when no workspace roots are available (e.g. no folder open, or the host
 * isn't ready yet) so callers can merge defensively.
 */
export async function getProjectMcpSettingsFilePaths(): Promise<string[]> {
	let workspaceRootPaths: string[] = []
	try {
		workspaceRootPaths = (await HostProvider.workspace.getWorkspacePaths({})).paths ?? []
	} catch {
		// Host not ready (e.g. unit tests) — no project sources.
		return []
	}
...
export async function getProjectSettingsDirectoryPath(): Promise<string> {
	try {
		...
		Logger.error("Failed to resolve project settings directory; falling back to global:", error)
	}
