...
// ---------------------------------------------------------------------------
// Code Search Protocol (CellockAI MCP-first)
// ---------------------------------------------------------------------------
...
export async function buildSessionConfig(input: SessionConfigInput): Promise<CoreSessionConfig> {
	...
	try {
		...
		// CellockAI: overlay the active workspace model profile (OpenAI-compatible)
		// so profile fields win. Non-fatal: no-op if no workspace is open or no
		// profiles file exists.
		apiConfig = overlayActiveProfile(stateManager.getApiConfiguration(), workspaceRoot)
