...
export class SdkTaskControlCoordinator {
	...
	async showTaskWithId(taskId: string): Promise<HistoryItem | undefined> {
		...
		try {
			...
			// opened task (see clearTaskSettings option doc). Dropping the overlay
			// before the transcript loads is safe: unlike clearing the old proxy's
			// messages or bumping the translator epoch, it does not produce
			// new-epoch empty snapshots.
			await this.options.clearTaskSettings()
