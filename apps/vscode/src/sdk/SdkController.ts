...
export class Controller {
	...
	constructor(readonly context: ClineExtensionContext) {
		...
		// CellockAI: MCP config is project-scoped — read/write all MCP servers
		// from <workspace>/.cellockai/mcp_settings.json instead of global storage.
	...
	private async createWorkspaceSessionHost(): Promise<VscodeSessionHost> {
		...
		// Temp hosts get the same enterprise-policy gate and prepared remote-config
		// integration as the shared session host (upstream behavior), while staying
		// workspace-scoped for the .cellockai session store.
	...
	async initTask(
		...
		// Ensure the initial (possibly org-managed) remote config is ready before
		// the session starts. Then fire-and-forget a refresh so the latest
		// enterprise policies (yoloModeAllowed, allowedMCPServers, etc.) are
